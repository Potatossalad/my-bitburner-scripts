  ...
  async function updateStocks() {
    ...
    for (const stock of stocks) {
      ...
      if (history.length > 0 && history[history.length - 1] !== 0) {
        ...
        const percentChange = ((stockPrice - lastPrice) / lastPrice) * 100;

        volatility.push(Math.abs(percentChange));
        if (percentChange > 0 ) {
          const increases = stockIncreases.get(stock) + 1;
          stockIncreases.set(stock, increases);
        }
        ns.print(`${stock} - % Change: ${percentChange.toFixed(2)}`);
      }
    ...
    if (updateCount % 10 === 0) {
      ...
      for (const [stock, increases] of sortedStocks) {
        ns.tprint(`${stocks} - Increases: ${increases}, Avg Volatility: ${average(stockVolatility.get(stock)).toFixed(2)}%`);
      }
      ...
      for (const stock of stocks) {
        ...
        if (position[0] > 0) {
          ...
          if (increases < 5) {
            ...
            ns.tprint(`Sold all ${position[0]} shares of ${stock} due to less than 5 increase in the last 10 updates.`);
            continue; // continues the next iteration to being immediately after selling
          }
          ...
          if (higherVolatilityStocks.length > 0) {
            ...
            higherVolatilityStocks.sort((a, b) => average(stockVolatility.get(b[0])) -
            average(stockVolatility.get(a[0])));
